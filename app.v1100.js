...
function cardHTML(vid, title, thumb, type='yt'){
  const href = type==='upload' ? `video.html?id=${encodeURIComponent(vid)}&type=upload` : `video.html?id=${encodeURIComponent(vid)}&type=youtube`;
...
async function loadHomeVideos(reset=false){
  ...
  try{
    ...
    for(const it of items){
      ...
      host.insertAdjacentHTML('beforeend', cardHTML(v,t,th,'yt'));
    }
