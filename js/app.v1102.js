...
async function fetchJSON(url){ const r=await fetch(url); const t=await r.text(); let d=null; try{d=JSON.parse(t);}catch(_){}
  if(!r.ok) throw new Error((d&&(d.error||JSON.stringify(d)))||t||('HTTP '+r.status)); return d||{}; }
async function loadHomeVideos(reset=false){
  const host=document.getElementById('home-videos'); const btnC=document.getElementById('load-more-container');
  if(!host) return;
  ...
  try{
    ...
    if(btnC) { btnC.innerHTML = nextPageToken ? '<button class="btn" id="moreBtn">Load more</button>' : ''; document.getElementById('moreBtn')?.addEventListener('click', ()=>loadHomeVideos(false)); }
  }catch(e){ if(reset && host) host.innerHTML='<div class="muted">Error loading videos.</div>'; console.error(e); }
}
async function loadDeenShorts(reset=false){
  const host=document.getElementById('shorts-container'); const btnC=document.getElementById('load-more-shorts-container');
  if(!host) return;
  ...
  try{
    ...
    if(btnC){ btnC.innerHTML = shortsPageToken ? '<button class="btn" id="moreShortsBtn">Load 50 more</button>' : ''; document.getElementById('moreShortsBtn')?.addEventListener('click', ()=>loadDeenShorts(false)); }
  }catch(e){ if(reset && host) host.innerHTML='<div class="muted">Error loading DeenShorts.</div>'; console.error(e); }
}
async function loadUploads(reset=false){
  const host=document.getElementById('uploads-container'); const btnC=document.getElementById('uploads-more-container');
  if(!host) return;
  ...
  try{
    ...
    if(btnC){ btnC.innerHTML = (uploadsPage!=null) ? '<button class="btn" id="moreUploadsBtn">Load more uploads</button>' : ''; document.getElementById('moreUploadsBtn')?.addEventListener('click', ()=>loadUploads(false)); }
  }catch(e){ if(reset && host) host.innerHTML='<div class="muted">Error loading uploads.</div>'; console.error(e); }
}
